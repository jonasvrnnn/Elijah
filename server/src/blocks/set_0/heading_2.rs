...
impl Block for Heading2 {
    ...
    fn render_to_noderef(&self) -> kuchikiki::NodeRef {
        ...
        let mut text_split = self.content.split("\n");
        let mut next = text_split.next();

        while let Some(current) = next {
            element.append(NodeRef::new_text(current));

            next = text_split.next();

            println!("{next:?}");

            if next.is_some() {
                let br = NodeRef::new_element(
                    QualName::new(None, ns!(html), LocalName::from("br")),
                    vec![],
                );
