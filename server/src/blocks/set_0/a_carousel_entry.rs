...
impl Block for ACarouselEntry {
    ...
    fn from_element_ref(element: &NodeDataRef<ElementData>) -> Result<Box<dyn Block>, BlockError> {
        ...
        let subtitle = attributes.get("subtitle").map(|v| v.to_string());
    ...
    fn render_to_noderef(&self) -> kuchikiki::NodeRef {
        let mut attributes = vec![
            (
                ...
                Attribute {
                    prefix: None,
                    value: "a-carousel-entry".to_string(),
                },
            ),
            (
        ...
        if let Some(subtitle) = &self.subtitle {
            attributes.push((
                ExpandedName::new(ns!(), LocalName::from("subtitle")),
                Attribute {
                    prefix: None,
    ...
    fn properties(&self) -> Markup {
        html!(
            form hx-put={"/api/blocks/"(self.id)"/properties"} hx-trigger="change" hx-on::after-request="refresh_preview()" hx-swap="none" {
                ...
                label {
                    "Title"
                    (properties::inner_text("title", &self.title, false))
                }
                label {
                    "Subtitle"
                    (properties::inner_text("sub-title", &self.subtitle, true))
                }
            }
        )
    }
    ...
    fn update(
        ...
        self.subtitle = properties.get("sub-title").cloned();
